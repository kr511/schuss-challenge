...
(function () {
  ...
  function cleanAuthUrl() {
    try { window.history.replaceState({}, document.title, getCanonicalRedirectUrl()); }
    catch (e) { console.warn('[AuthGate] Could not clean auth URL:', e); }
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #authGate.fade-out{opacity:0;pointer-events:none}.ag-card{background:#13131f;border:1px solid #1f2937;border-radius:20px;padding:40px 36px;width:100%;max-width:400px;margin:16px;box-shadow:0 24px 64px rgba(0,0,0,.6)}
      ...
      .ag-divider{display:flex;align-items:center;gap:10px;color:#374151;font-size:13px;margin-bottom:16px}.ag-divider:before,.ag-divider:after{content:'';flex:1;height:1px;background:#1f2937}.ag-google{width:100%;padding:12px;background:#fff;border:0;border-radius:10px;font-size:15px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;transition:opacity .2s}.ag-google:hover:not(:disabled){opacity:.9}.ag-google:disabled{opacity:.5;cursor:not-allowed}.ag-google-icon{width:20px;height:20px}
      .ag-error{background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.3);color:#fca5a5;border-radius:8px;padding:10px 14px;font-size:13px;margin-bottom:14px;display:none}.ag-info{background:rgba(122,176,48,.12);border:1px solid rgba(122,176,48,.3);color:#a3d65a;border-radius:8px;padding:10px 14px;font-size:13px;margin-bottom:14px;display:none;text-align:center}.ag-spinner{text-align:center;padding:20px;color:#6b7280;font-size:14px}.ag-form{display:none}
    `;
    document.head.appendChild(style);
  }

  function createGate() {
    if (document.getElementById('authGate')) { gateEl = document.getElementById('authGate'); return; }
    gateEl = document.createElement('div');
    gateEl.id = 'authGate';
    gateEl.innerHTML = `
      <div class="ag-card"><div class="ag-logo">🎯</div><div class="ag-title">SCHUSSDUELL</div><div class="ag-sub">Melde dich an um loszulegen</div><div id="agSpinner" class="ag-spinner">⏳ Prüfe Anmeldung…</div><div id="agForm" class="ag-form"><div class="ag-tabs"><button class="ag-tab active" id="agTabSignin" type="button" onclick="__agSetMode('signin')">Anmelden</button><button class="ag-tab" id="agTabSignup" type="button" onclick="__agSetMode('signup')">Registrieren</button></div><div id="agError" class="ag-error"></div><div id="agInfo" class="ag-info"></div><label class="ag-label" for="agEmail">E-Mail-Adresse</label><input id="agEmail" class="ag-input" type="email" placeholder="name@example.com" autocomplete="email"><label class="ag-label" for="agPassword">Passwort</label><input id="agPassword" class="ag-input" type="password" placeholder="Mindestens 6 Zeichen" autocomplete="current-password"><button id="agSubmit" class="ag-btn" type="button" onclick="__agSubmit()">Anmelden</button><div class="ag-divider">oder</div><button id="agGoogle" class="ag-google" type="button" onclick="__agGoogle()"><span aria-hidden="true">G</span> Mit Google anmelden</button></div></div>`;
    document.body.insertBefore(gateEl, document.body.firstChild);
    const onKey = (event) => { if (event.key === 'Enter') window.__agSubmit(); };
  ...
  function hideGate(immediate = false) {
    if (!gateEl) return;
    if (immediate) { if (gateEl.parentElement) gateEl.remove(); return; }
    gateEl.classList.add('fade-out');
    setTimeout(() => { if (gateEl && gateEl.parentElement) gateEl.remove(); }, 420);
  }
  ...
  function exposeSession(session) {
    ...
    } catch (e) { console.warn('[AuthGate] Username prefill skipped:', e); }
  }

  function reloadCleanlyAfterSignin() {
    setTimeout(() => window.location.replace(getCanonicalRedirectUrl()), 80);
  }
  ...
  function waitForSupabase(timeoutMs = 10000) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setInterval(() => {
        if (window.supabase && typeof window.supabase.createClient === 'function') { clearInterval(timer); resolve(window.supabase); return; }
        if (Date.now() - started >= timeoutMs) { clearInterval(timer); reject(new Error('Supabase konnte nicht geladen werden.')); }
      }, 50);
    });
  }
  ...
  window.__agSubmit = async function () {
    if (busy) return;
    if (!client) { showError('Anmeldung ist noch nicht bereit. Bitte kurz warten und erneut versuchen.'); return; }
    ...
    try {
      ...
      setBusy(false); authFinishing = false;
    }
  };

  window.__agGoogle = async function () {
    if (busy) return;
    if (!client) { showError('Google-Anmeldung ist noch nicht bereit. Bitte kurz warten und erneut versuchen.'); return; }
    setBusy(true); showError(''); showInfo('Weiterleitung zu Google…');
    try {
      const { error } = await client.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: getCanonicalRedirectUrl(), queryParams: { prompt: 'select_account' } }
      });
      if (error) throw error;
    } catch (err) {
  ...
  async function init() {
    ...
    initialized = true;
    injectStyles(); createGate(); setBusy(true);
    try {
      ...
      if (oauthSession) { onAuthenticated(oauthSession, { reloadAfterSignin: true }); return; }
      client.auth.onAuthStateChange((event, session) => { if ((event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') && session) onAuthenticated(session); });
      const session = await getStoredSession();
      if (session) { onAuthenticated(session); return; }
      showForm();
    } catch (err) {
