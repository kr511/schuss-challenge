-- Schussduell Supabase Social Schema
-- Run this once in Supabase SQL Editor when moving friends/challenges from Firebase to Supabase.
...
-- Profiles: users can read profiles and manage their own profile.
...
-- Friend codes: readable so codes can be resolved; users manage only their own code.
...
-- Friend requests: sender and receiver can see; sender can create; receiver can update status.
...
-- Friends: both sides can read their own friend rows; app writes rows for the current user.
...
-- Online status: everyone can read, users update their own status.
...
-- Challenges/results: only creator/opponent can read/write their challenge data.
...
-- Helper: create/update profile from currently authenticated user.
...
-- Helper: accept a friend request and create both friendship rows atomically.
